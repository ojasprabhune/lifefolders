...
- **commands.rs** — apply logic for the command tool group: reschedule/clear-due-date/status/recategorize/delete a task, start a focus session, plan the day, delete the last entry. Returns a notice, never a `logs` row (the exception being the task-history rows `write_history` writes). `plan_today` is the one command that reaches outside the tasks domain: it feeds the clock, the open sidequests, and `focus::minutes_by_task` to `groq::plan_today` and prepends the result to `daily_notes.today_text`. Two things about its brief are load-bearing and were both learned by watching it get them wrong. The sidequests are **split into two labelled groups** (due-today-or-late, and not-due-yet) rather than sorted by date — given one flat list the model spread the whole backlog across the evening, and which side of the line something falls on has to be stated, not inferred. And every sidequest carries a length, taken from `effort_minutes` or, failing that, parsed out of its note by `tasks::effort_from_text` — people wrote estimates into notes before there was a field for them. Without a length the model gave everything a flat thirty minutes, which is how three hours of research got scheduled into half an hour. Meal gaps go in the brief as a `KEEP FREE` block built from `MEAL_BREAKS`, again as data rather than prose, so moving a meal is an edit rather than a reworded instruction. A break is the one thing allowed to interrupt a block — the sidequest resumes after it for the time it still needs, which is not the same as shortening it, and the prompt has to say so explicitly or the model treats the two as the same rule.
