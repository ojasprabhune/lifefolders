...
`roll` is the one reveal with a backend half. `TaskData` carries `previous_due_date`, set only on a reschedule, so the timeline row can say `monday → friday` rather than only where it landed. It's inside `logs.data`, so no migration — but it is `skip_serializing_if = "Option::is_none"`, and rows written before it existed simply don't have it, which is why `revealOf` falls back to `decode` when it's absent.
