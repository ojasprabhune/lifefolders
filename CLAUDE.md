...
`#/sleep/all` is the second wall, same shape as the cadence one and reusing the same `.app.wall` container. It is the only place using **anime.js** (v4, named exports; ~20kb gzipped): one `createTimeline` staggers the panels, raster bars, histogram bars and calendar cells in, `svg.createDrawable` draws the line charts, and numbers count up via `animate()` on a plain object. Two rules that page has to keep: every selector is scoped to the page root via `root.querySelectorAll` rather than a bare string, and nothing anime animates may also carry meaning in the same property — the calendar's four shading levels moved from `opacity` to `color-mix` precisely because the entrance animation was writing `opacity: 1` over them. Under `prefers-reduced-motion` the timeline is never built and the `pre-anim` class is simply dropped.
